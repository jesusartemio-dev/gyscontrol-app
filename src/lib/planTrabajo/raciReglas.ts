 ...
 * Semilla (informe §6 / cambio #13):
 *   Gerente*                          → A en todo
 *   Gestor/Supervisor de Proyecto*     → R en EDTs de gestión, C en EDTs de campo
 *   Supervisor de campo/Construcción*  → R en EDTs de campo, I en EDTs de gestión
 *   Técnico*                          → R en el EDT del que es responsable, I en el resto
 *   *Seguridad*                       → C en todo
...
export const REGLAS_RACI_CARGO: ReglaRaci[] = [
  { patron: /gerente/i, calcular: () => 'A' },
  { patron: /(gestor|supervisor\s+de\s+proyecto)/i, calcular: ctx => (ctx.tipoEdt === 'gestion' ? 'R' : 'C') },
  { patron: /(supervisor\s+de\s+campo|construcci[oó]n)/i, calcular: ctx => (ctx.tipoEdt === 'campo' ? 'R' : 'I') },
  { patron: /t[eé]cnico/i, calcular: ctx => (ctx.esResponsableDelEdt ? 'R' : 'I') },
  { patron: /seguridad/i, calcular: () => 'C' },
