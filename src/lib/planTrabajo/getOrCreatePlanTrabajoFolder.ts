...
/** Busca la carpeta PlanTrabajo_<codigo> en el SharedDrive; la crea si no existe. */
export async function getOrCreatePlanTrabajoFolder(proyectoCodigo: string): Promise<string> {
  const parentId = getSharedDriveId()
  const folderName = `PlanTrabajo_${proyectoCodigo}`
