...
| `{#tieneHistogramaHHActividadPng}` → `{%histogramaHHActividadPng}` (§13.3, ya insertada en la plantilla) | Detalle de HH por actividad, SOLO EDTs de Construcción/Comisionamiento (`esEdtDeConstruccionOComisionamiento`, raciReglas.ts), series por CARGO real (mismo vocabulario que `equipoTrabajo`, nunca "Técnico 1/2/3" — el modelo no distingue instancias). Es un COMPLEMENTO de `histogramaHHPng` (§13.2, por EDT/mes) — mismo total HH real que §1/§2/§13.2/§14 (triple igualdad, ver calcularDatos.ts). Compuerta de cobertura: si no el 100% de las tareas con recurso de esos EDTs tienen horasEstimadas cargada, la sección entera se omite (nunca un gráfico parcial) y Etapa 1 avisa |
