...
> ⚠️ **La plantilla v5 (Bloque 4.2 sesión 2) eliminó el loop `{#imagenes}` a nivel subItem.**
> La UI (editor y vista) sigue permitiendo adjuntar imágenes a un subItem
> (`GaleriaImagenesAlcance`/`GaleriaSoloLectura` con `subItemRef`, sin `tareaRef`)
> y `construirDataBag.ts` las sigue calculando en `subItems[].imagenes` — pero
> como la plantilla ya no tiene ese tag, **esas imágenes no aparecen en el docx
> exportado**, solo en la app. Si se necesita que vuelvan a exportarse, hay que
> reintroducir `{#imagenes}{%img}{caption}{/imagenes}` dentro de `{#subItems}`
> en el `.docx` (el dataBag ya las provee, no requiere cambios de código).
