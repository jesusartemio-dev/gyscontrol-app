...
| `{#tieneHistogramaHHActividadPng}` → `{%histogramaHHActividadPng}` (§13.2 — NUEVO, falta agregar el párrafo en la plantilla) | Código listo del lado dataBag/cálculo (informe §13.2): detalle de HH por actividad, SOLO EDTs de Construcción/Comisionamiento (`esEdtDeConstruccionOComisionamiento`, raciReglas.ts), series por CARGO real (mismo vocabulario que `equipoTrabajo`, nunca "Técnico 1/2/3" — el modelo no distingue instancias). Es un COMPLEMENTO de `histogramaHHPng` (que sigue siendo por EDT/mes, tabla de 468 HH intacta), no un total alternativo — **falta insertar en Word**, mismo patrón que los otros dos: un párrafo con `{#tieneHistogramaHHActividadPng}`, uno con `{%histogramaHHActividadPng}` centrado, y uno de cierre `{/tieneHistogramaHHActividadPng}`. Sugerencia de heading: "13.3. Detalle de Horas-Hombre por Actividad — Construcción y Comisionamiento" (13.2 ya está tomado por el histograma HH por EDT existente) |
