...
/** Busca la carpeta IPERC_<codigo> en el SharedDrive; la crea si no existe. */
export async function getOrCreateIpercFolder(proyectoCodigo: string): Promise<string> {
  const parentId = getSharedDriveId()
  const folderName = `IPERC_${proyectoCodigo}`
