...
/** Busca la carpeta MPP_<codigo> en el SharedDrive; la crea si no existe. */
export async function getOrCreateMppFolder(proyectoCodigo: string): Promise<string> {
  const parentId = getSharedDriveId()
  const folderName = `MPP_${proyectoCodigo}`
