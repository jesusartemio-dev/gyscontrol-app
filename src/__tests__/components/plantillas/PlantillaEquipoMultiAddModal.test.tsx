/**
 * @fileoverview Tests para PlantillaEquipoMultiAddModal
...
jest.mock('@/lib/services/plantillaEquipoItem', () => ({
  createPlantillaEquipoItem: jest.fn().mockResolvedValue({ id: 'test' }),
}));
...
describe('PlantillaEquipoMultiAddModal', () => {
  ...
  it('should import component without errors', () => {
    expect(PlantillaEquipoMultiAddModal).toBeDefined();
    expect(typeof PlantillaEquipoMultiAddModal).toBe('function');
  });

  it('component should be a valid React component', () => {
    expect(PlantillaEquipoMultiAddModal.prototype).toBeDefined();
  });
  ...
  it('has mocked services available', () => {
    ...
    const { createPlantillaEquipoItem } = require('@/lib/services/plantillaEquipoItem');

    expect(getCatalogoEquipos).toBeDefined();
    expect(getCategoriasEquipo).toBeDefined();
    expect(createPlantillaEquipoItem).toBeDefined();
  });
  ...
  it('can call mocked service functions', async () => {
    ...
    const created = await createPlantillaEquipoItem({});
  ...
  it('accepts expected prop types', () => {
    ...
    expect(typeof mockProps.plantillaEquipoId).toBe('string');
