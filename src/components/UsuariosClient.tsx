...
export const roleDisplayMap: Record<RolUsuario, { label: string; color: string; bgColor: string; description: string }> = {
  ...
  administracion: {
    ...
    description: 'Gestión financiera, depósitos y rendiciones'
  },
  colaborador: {
    label: 'Colaborador',
    color: 'text-gray-700',
  ...
  rrhh: {
    ...
    description: 'Personal y asistencia — sin las secciones financieras de Administración'
  },
}
...
export default function UsuariosClient() {
  ...
  const stats = useMemo(() => ({
    ...
    google: usuarios.filter(u => u.authProviders?.includes('google')).length,
    credentials: usuarios.filter(u => u.hasPassword).length,
              ...
              <TableCell>
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-1">
                    {usuario.authProviders?.includes('google') && (
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-5 bg-white border-gray-200 gap-1">
                        ...
                        <TooltipContent>Vinculado con Google</TooltipContent>
                      </Tooltip>
                    )}
                    {usuario.hasPassword && (
                      <Tooltip>
                        ...
                        <TooltipContent>Tiene contraseña del sistema</TooltipContent>
                      </Tooltip>
                    )}
                    {!usuario.hasPassword && !usuario.authProviders?.length && (
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-5 bg-amber-50 border-amber-200 text-amber-700">
                            Sin acceso
                          </Badge>
                        </TooltipTrigger>
                        <TooltipContent>No tiene método de login configurado</TooltipContent>
                      </Tooltip>
                    )}
                  </div>
                </div>
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-6">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-600">{stats.credentials}</div>
