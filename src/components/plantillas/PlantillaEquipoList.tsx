...
import { deletePlantillaEquipo, updatePlantillaEquipo } from '@/lib/services/plantillaEquipo'
...
interface Props {
  ...
  onUpdated: (actualizado: PlantillaEquipo) => void
}

export default function PlantillaEquipoList({ equipos, plantillaId, onDeleted, onUpdated }: Props) {
  ...
  const handleEdit = async (id: string, field: string, value: string) => {
    ...
    setLoadingId(id)
    try {
      const actualizado = await updatePlantillaEquipo(id, { [field]: value })
