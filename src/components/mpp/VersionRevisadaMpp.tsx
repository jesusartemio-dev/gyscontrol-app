...
import { FileSpreadsheet, ExternalLink, Loader2, FolderOpen } from 'lucide-react'
...
/**
 * Vista de la última versión IMPORTADO vigente de la MPP (el Excel que
 * alguien revisó/aprobó fuera de la app y volvió a subir), renderizada TAL
 * CUAL — mismo patrón que VersionRevisadaIperc.tsx. No se re-parsea a
 * `MppItem` — es una vista de solo lectura del archivo.
 */
export function VersionRevisadaMpp({ proyectoId }: Props) {
  ...
  if (!data) {
    return (
      <p className="text-sm text-muted-foreground text-center py-8">
        Todavía no se subió ninguna versión revisada de esta MPP.
      </p>
    )
  }
