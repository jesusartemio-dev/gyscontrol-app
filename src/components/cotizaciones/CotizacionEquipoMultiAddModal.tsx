...
import type { CatalogoEquipo, CotizacionEquipoItem, CategoriaEquipo } from '@/types'
...
export default function CotizacionEquipoMultiAddModal({
  isOpen,
  onClose,
  cotizacionEquipoId,
  onItemsCreated
  ...
  const getAvailableCategorias = () => {
    if (marcaFiltro === 'todas') {
      return categorias.filter(categoria => 
        equipos.some(equipo => equipo.categoriaId === categoria.id)
      )
    }
    ...
    return categorias.filter(categoria => 
      categoriasConMarca.includes(categoria.id!)
    )
  }
  ...
  // 🔁 Reset marca filter when categoria changes and selected marca is not available
  useEffect(() => {
    if (categoriaFiltro !== 'todas' && marcaFiltro !== 'todas') {
      const availableMarcas = getAvailableMarcas()
      if (!availableMarcas.includes(marcaFiltro)) {
        setMarcaFiltro('todas')
      }
    }
  }, [categoriaFiltro, equipos])

  // 🔁 Reset categoria filter when marca changes and selected categoria is not available
  useEffect(() => {
    if (marcaFiltro !== 'todas' && categoriaFiltro !== 'todas') {
      const availableCategorias = getAvailableCategorias()
      if (!availableCategorias.some(cat => cat.id === categoriaFiltro)) {
  ...
  // ✅ Save selected equipos as cotizacion items
  const handleSave = async () => {
    ...
    try {
      setSaving(true)
      const createdItems: CotizacionEquipoItem[] = []
      ...
      for (const selectedEquipo of selectedEquipos) {
        ...
        const createdItem = await createCotizacionEquipoItem(itemData)
