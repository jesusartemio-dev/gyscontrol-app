...
export default function CotizacionGastoItemTable({ items, onUpdate, onDelete, isLocked = false }: Props) {
  ...
  const [editItem, setEditItem] = useState<CotizacionGastoItem | null>(null)
  const [itemToDelete, setItemToDelete] = useState<CotizacionGastoItem | null>(null)
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y">
                {filteredItems.map((item, idx) => {
                  const margenPct = item.costoInterno > 0
