...
/**
 * Vista de la última versión IMPORTADO vigente del PETS (el Word que alguien
 * revisó/aprobó fuera de la app y volvió a subir), renderizada TAL CUAL —
 * mismo patrón que VersionRevisadaView.tsx del Plan de Trabajo. No se
 * re-parsea a `contenido` — es una vista de solo lectura del archivo.
 */
export function VersionRevisadaPets({ proyectoId }: Props) {
  ...
  if (!data) {
    return (
      <p className="text-sm text-muted-foreground text-center py-8">
        Todavía no se subió ninguna versión revisada de este PETS.
      </p>
    )
  }
