...
interface JornadaActiva {
  ...
  tareas: TareaJornada[]
  evidenciaAvance?: { id: string; estado: string } | null
}
...
const STORAGE_KEY = 'proyectos:ultimaJornadaId'
...
export function SelectorJornada({ value, onChange, filtroProyectoId, filtroFechaDesde, filtroFechaHasta }: Props) {
  ...
  const query = useQuery<JornadaActiva[]>({
    queryKey: ['proyectos', 'jornadas-activas', soloAsignadas, filtroProyectoId, filtroFechaDesde, filtroFechaHasta],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/proyectos/registros-evidencia/jornadas-activas?${params.toString()}`, {
        credentials: 'include',
      })
  ...
  return (
    <Card className="p-3 space-y-2">
      ...
      {enModoFiltrado && (
        <p className="text-[11px] text-orange-600">Filtrado al rango indicado.</p>
      )}
