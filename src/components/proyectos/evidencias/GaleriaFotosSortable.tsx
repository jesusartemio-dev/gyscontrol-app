...
export function GaleriaFotosSortable({ registroId, fotos, editable }: Props) {
  ...
  const reorderMutation = useMutation({
    mutationFn: async (ordenes: { id: string; orden: number }[]) => {
      const res = await fetch(`/api/proyectos/registros-evidencia/${registroId}/fotos/orden`, {
        method: 'PATCH',
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['proyectos', 'registro', registroId] })
    },
    onError: (e, _vars, ctx) => {
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between">
        <p className="text-[11px] text-muted-foreground">
          Arrastra para reordenar · La primera se usa como portada
        </p>
        {!lleno && (
...
function FotoItem({ foto, onDelete, deleting }: FotoItemProps) {
  ...
  return (
    ...
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        src={`/api/proyectos/registros-evidencia/fotos/${foto.id}/contenido`}
      ...
      />
      <a
        href={`/api/proyectos/registros-evidencia/fotos/${foto.id}/contenido`}
        target="_blank"
        rel="noopener noreferrer"
