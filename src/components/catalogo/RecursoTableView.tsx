...
export default function RecursoTableView({ data, onEdit, onDelete, loading = false, error = null }: Props) {
    ...
    <TooltipProvider>
      <div className="border rounded-lg bg-white overflow-hidden">
        <Table>
          ...
          <TableBody>
            {data.map((recurso) => {
              // Calcular costos para comparación
              const composiciones = recurso.composiciones || []
              const tienePersonal = composiciones.length > 0

              // Para CUADRILLAS: suma de costos (ponderada por % de dedicación)
              // Para INDIVIDUALES: promedio de costos (referencia)
              ...
              // Positivo = Recurso cuesta más que el real (ganancia)
              // Negativo = Recurso cuesta menos que el real (pérdida)
              const getEstadoColor = () => {
                if (!tienePersonal) return 'text-gray-400'
                if (Math.abs(porcentajeDif) < 5) return 'text-green-600' // OK
                if (diferencia > 0) return 'text-green-600' // Ganancia
                return 'text-red-600' // Pérdida
              }
