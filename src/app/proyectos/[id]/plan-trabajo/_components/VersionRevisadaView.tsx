...
/**
 * Vista de la última versión IMPORTADO vigente (el Word que Proyectos editó a
 * mano y volvió a subir), renderizada TAL CUAL — texto, imágenes, captions y
 * posición exactos del documento. A propósito NO intenta mapear nada de
 * vuelta a la estructura de la app: no hay ancla estable para hacerlo de
 * forma confiable (se edita en Google Docs, que borra cualquier marca
 * oculta) — ver project_plan_trabajo_v2_upload.md.
 */
export function VersionRevisadaView({ proyectoId }: Props) {
  ...
  useEffect(() => {
    ...
    setCargando(true)
    setError(null)
    fetch(`/api/proyectos/${proyectoId}/plan-trabajo/version-revisada`)
      .then(async res => {
  ...
  if (!data) {
    return (
      <p className="text-sm text-muted-foreground text-center py-8">
        Todavía no se subió ninguna versión revisada de este Plan de Trabajo.
      </p>
    )
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border rounded-md px-3 py-2 bg-muted/40 text-sm">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="font-medium truncate">{data.codigoNexa ?? data.archivoNombre}</span>
        </div>
        <span className="text-xs text-muted-foreground">Rev. {data.numeroRevision}</span>
        <span className="text-xs text-muted-foreground">
          {new Date(data.generadoEn).toLocaleString('es-PE', {
      ...
      <div className="overflow-x-auto border rounded-md p-4 bg-white">
        ...
        <div className="docx-vista" dangerouslySetInnerHTML={{ __html: data.html }} />
      </div>
    </div>
  )
}
