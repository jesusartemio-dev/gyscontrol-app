  ...
  const cargarResumenProyectos = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.log('🔍 Frontend: Resumen General:', data.data.resumenGeneral)
        console.log('🔍 Frontend: Proyectos encontrados:', data.data.proyectos.length)
        data.data.proyectos.forEach((proyecto: any, index: number) => {
          console.log(`   Proyecto ${index + 1}: ${proyecto.proyecto.codigo} - ${proyecto.proyecto.nombre}`)
          console.log(`   Servicios en proyecto: ${proyecto.servicios.length}`)
          proyecto.servicios.forEach((servicio: any, servicioIndex: number) => {
            console.log(`     Servicio ${servicioIndex + 1}: ${servicio.nombre} (${servicio.horasEstimadas}h estimadas, ${servicio.categoria})`)
          })
        })
      } else {
          ...
          <CardContent className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">Sin Planificación:</span>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs text-gray-600">
                        {mostrarTodosEdts ? 'Todos los Servicios:' : 'Servicios Principales (TOP 3):'}
                      </p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                      {proyecto.servicios.map((servicio: any, index: number) => (
                        <div
                          key={index}
