...
export default function AsistenciaCampoPage() {
  ...
  async function iniciarJornada() {
    ...
    if (!coords) {
      toast.error('Se requiere GPS para abrir la asistencia de campo')
      return
    }
    setLoading(true)
    try {
      ...
      if (!r.ok) {
        toast.error(j.message || 'Error al abrir asistencia')
        return
      }
  ...
  async function cerrarJornada() {
    if (!jornada) return
    if (!confirm('¿Cerrar asistencia de campo? Los trabajadores ya no podrán marcar con este QR.')) return
    setLoading(true)
    try {
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 py-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">Asistencia de Campo</h1>
        <p className="text-sm text-muted-foreground">
          Activa el QR del día en tu ubicación para que tu equipo (y tú) pueda marcar ingreso y salida
        </p>
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Play className="h-5 w-5" /> Abrir asistencia de campo
            </CardTitle>
          </CardHeader>
