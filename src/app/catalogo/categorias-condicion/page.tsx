...
import { exportarCategoriasCondicionAExcel } from '@/lib/utils/categoriaCondicionExcel'
import { leerCategoriasCondicionDesdeExcel, validarCategoriasCondicion } from '@/lib/utils/categoriaCondicionImportUtils'
...
export default function CategoriasCondicionPage() {
  ...
  const cargarDatos = async () => {
    setLoading(true)
    try {
      const data = await getCategoriasCondicion()
      setCategorias(data)
    } catch (error) {
  ...
  const handleCreate = async () => {
    ...
    setSaving(true)
    try {
      await createCategoriaCondicion({
  ...
  const handleExportar = () => {
    try {
      exportarCategoriasCondicionAExcel(categorias)
  ...
  const handleImportar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const datos = await leerCategoriasCondicionDesdeExcel(file)
      const nombresExistentes = categorias.map(c => c.nombre)
      const { nuevas, errores, duplicados } = validarCategoriasCondicion(datos, nombresExistentes)
      ...
      await Promise.all(nuevas.map(c => createCategoriaCondicion({
        nombre: c.nombre,
