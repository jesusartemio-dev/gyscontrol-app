...
export async function POST(
  ...
  // Pre-flight checks (returned as JSON before stream starts)
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        trackUsage({
          userId,
          tipo: 'tdr-analisis-pdf-extraccion',
          modelo: sonnetModel,
        ...
        if (!extracted) {
          console.error('[analizar-pdf] Fase 2 JSON parse failed. Raw (500):', rawText.slice(0, 500))
        ...
        let analisis = await prisma.cotizacionTdrAnalisis.findFirst({
          where: { cotizacionId },
        ...
        send('done', { analisis: final })
      } catch (error) {
        console.error('[analizar-pdf]', error)
