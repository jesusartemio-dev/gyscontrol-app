...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (existing.estado !== 'enviada') {
      return NextResponse.json({ error: 'Solo se puede confirmar desde estado enviada' }, { status: 400 })
    }
    ...
    const data = await prisma.ordenCompra.update({
      ...
      data: {
        ...
        fechaEntregaEstimada: body.fechaEntregaEstimada ? new Date(body.fechaEntregaEstimada) : existing.fechaEntregaEstimada,
        updatedAt: new Date(),
      },
      include: {
        proveedor: true,
    ...
    return NextResponse.json({ error: 'Error al confirmar orden de compra' }, { status: 500 })
  }
}
