...
import { z } from 'zod'
import { regenerarPasoPets } from '@/lib/pets/regenerarConIa'
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: 'etapaIndex y pasoIndex requeridos (números enteros ≥ 0)' },
      { status: 400 },
    )
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        for await (const evento of regenerarPasoPets(
          proyectoId,
          parsed.data.etapaIndex,
          parsed.data.pasoIndex,
          session.user.id,
        )) {
          send(evento)
