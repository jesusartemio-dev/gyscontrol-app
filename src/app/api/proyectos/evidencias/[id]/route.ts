...
import { actualizarEvidenciaAvanceSchema } from '@/lib/validators/evidenciaAvance'
import {
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('Error al obtener evidencia de avance:', error)
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = actualizarEvidenciaAvanceSchema.safeParse(body)
    if (!parsed.success) {
    ...
    if (parsed.data.observaciones !== undefined) {
      const actualizada = await prisma.evidenciaAvance.update({
        where: { id },
    ...
    console.error('Error al actualizar evidencia de avance:', error)
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.evidenciaAvance.delete({ where: { id } })
    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Error al eliminar evidencia de avance:', error)
